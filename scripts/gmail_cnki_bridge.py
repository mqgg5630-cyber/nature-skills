...
def build_review_delivery_message(
    ...
    lines.append("【你的任务】")
    lines.append("=" * 60)
    lines.append("你收到的是一份【自动机械汇编的综述底本草稿】，不是终稿。")
    lines.append("它由本批次入库文献的结构化卡片拼装而成，未经人工核验；")
    lines.append("凡标注「待核」的字段表示未能从 PDF 原文抽取，请勿直接采信。")
    lines.append("若发现正文出现与本批次主题无关的内容或文献，请在审计报告中明确指出。")
    lines.append("请先做一致性校验，再按 ARTA 五支柱逐条审计，输出修订后的版本：")
    lines.append(f"  0. 主题一致性：正文内容是否确实围绕「{topic}」？")
    lines.append("     若出现其它领域的机制、数值或作者（说明生成端串了模板），")
    lines.append("     请判定为不通过并列出全部错位内容，再据实重写。")
