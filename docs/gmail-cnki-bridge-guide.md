...
## 十七、⚠️ 综述底本是「草稿」，不是终稿（重要修正）

### 曾经的严重缺陷

`synthesize_thesis_chapter1()` 早期版本是一篇**写死的鲜味肽范文**：无论你的批次是什么主题，
生成的绪论都会注入 T1R1/T1R3 受体机制、"降盐 30%"、"小分子肽 84.5%"、
"感官阈值 0.12 mg/mL"，以及「陈峰/赵伟/黄婷/王芳」等**并不属于该批次的作者**。

这导致「深度学习筛选抗菌肽」批次产出的综述里出现了整段食品鲜味肽内容——
属于**凭空捏造**，直接违反 ARTA 的"零虚假引用"红线。

同时 `PaperCardBuilder.build()` 对未收录条目返回的兜底值是
"具有统计学显著性改善。"、"特定实验体系限制。"——看起来像真结论，其实是占位符。

### 现在的行为

- **主题无关的批次走通用骨架**：只机械汇编本批次卡片里**真实存在**的信息，
  不注入任何领域范文；参考文献严格与本批次 `item_key` 一一对应。
- **抽取不到的一律标「待核」**，不再输出伪装成结论的占位符。
- **底本开头自带警示**："本文档为自动编译的综述底本（草稿），不是终稿，未经人工核验。"
- **鲜味肽主题**（topic 或标题命中 `鲜味/umami/T1R1/呈味/减盐` 等关键词）
  仍走原领域模板，既有行为不受影响。
- **交付邮件新增「第 0 条：主题一致性校验」**，要求 Spark 先检查正文是否确实围绕本批次主题，
  发现串模板就判定不通过并列出错位内容。

### 你该怎么用

自动底本的定位是**给 Spark 的原料**，不是可交稿的绪论。真正的横向论证
（方法演进主线、指标可比性、矛盾结论辨析、研究空白）必须由 Spark 审稿环节
基于 1.3 节的逐篇真实卡片重构完成——这正是 Spark 在 batch_2 审计里做的事。

> 提示：Spark 对 batch_2 的审计结论（主题错位、因果断裂、虚假引用）**完全正确**，
> 它发现的是本地生成器的 bug。这一版修复后，同类问题不会再发生。
